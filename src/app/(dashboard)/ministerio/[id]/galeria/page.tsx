...
export default function MinistryGalleryPage({ params }: { params: { id: string } }) {
  ...
  async function fetchAlbums(): Promise<Album[]> {
    setLoading(true)
    try {
      const res = await fetch(`/api/gallery/albums?ministryId=${ministryId}`)
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/ministerio/${ministryId}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d?.name) setMinistryName(d.name) })
  ...
  return (
    <>
      <style suppressHydrationWarning>{`
        ...
        .btn-primary { display:inline-flex; align-items:center; gap:6px; background:#16a34a; color:#fff; border:none; border-radius:10px; padding:10px 20px; font-size:14px; font-weight:600; cursor:pointer; transition: opacity .15s, transform .15s; font-family:'DM Sans',sans-serif; }
        ...
        .drop-zone.over { border-color: #16a34a; background: #f0fdf4; }
      ...
      <div className="gal bg-gray-50 min-h-screen">

        {/* HERO */}
        <div style={{ background: ad }}>
          <div className="px-6 md:px-10 pt-6 pb-8">
            <Link href={`/ministerio/${ministryId}`} className="inline-flex items-center gap-1.5 text-white/40 hover:text-white/70 text-xs transition mb-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                ...
                <p className="text-white/40 text-sm">
                  {ministryName || "Ministério"} — álbuns de fotos
                </p>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 fade-in" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input className="inp" placeholder="Ex: Louvor 2025" value={newAlbumTitle} onChange={e => setNewAlbumTitle(e.target.value)} onKeyDown={e => e.key === "Enter" && handleCreateAlbum()} autoFocus />
                </div>
                <div>
        ...
        {showAddPhoto && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop" style={{ background: "rgba(0,0,0,0.5)" }}
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 fade-in" onClick={e => e.stopPropagation()}>
              ...
              {previews.length === 0 ? (
                <div className="drop-zone p-10 flex flex-col items-center justify-center gap-3 cursor-pointer"
                  ...
                  <button className="btn-primary" style={{ fontSize: 13, padding: "8px 18px" }}>Selecionar do computador</button>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input className="inp" placeholder="Ex: Culto de louvor 2025" value={photoCaption} onChange={e => setPhotoCaption(e.target.value)} disabled={addingPhoto} />
                  </div>
        ...
        {lightboxPhoto && activeAlbum && (
          <div className="fixed inset-0 z-50 flex items-center justify-center fade-in" style={{ background: "rgba(0,0,0,0.92)" }} onClick={() => setLightboxPhoto(null)}>
            <button className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 text-white flex items-center justify-center hover:bg-white/20 transition" onClick={() => setLightboxPhoto(null)}>
              <X size={18} />
            </button>
            {activeAlbum.photos.length > 1 && (
              <button className="lightbox-nav absolute left-4 w-11 h-11 rounded-full bg-white/10 text-white flex items-center justify-center" onClick={e => { e.stopPropagation(); lightboxPrev() }}>
                <ChevronLeft size={20} />
              </button>
            )}
            ...
            {activeAlbum.photos.length > 1 && (
              <button className="lightbox-nav absolute right-4 w-11 h-11 rounded-full bg-white/10 text-white flex items-center justify-center" onClick={e => { e.stopPropagation(); lightboxNext() }}>
