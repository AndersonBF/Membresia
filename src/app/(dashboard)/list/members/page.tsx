...
const MemberListPage = async ({
  ...
  const columns = [
    ...
    { header: "Grupos", accessor: "groups", className: "hidden md:table-cell" },
    { header: "Gênero", accessor: "gender", className: "hidden md:table-cell" },
    { header: "Data Nascimento", accessor: "birthDate", className: "hidden lg:table-cell" },
      ...
      <td className="hidden md:table-cell">
        <div className="flex flex-wrap gap-1">
          {item.societies?.map((s) => (
            <span key={s.societyId} className="px-2 py-0.5 rounded-full text-xs bg-blue-100 text-blue-800 font-medium">
              {s.society.name}
            </span>
          ))}
          {item.council && (
            <span className="px-2 py-0.5 rounded-full text-xs bg-indigo-100 text-indigo-800 font-medium">Conselho</span>
          )}
          {item.diaconate && (
            <span className="px-2 py-0.5 rounded-full text-xs bg-teal-100 text-teal-800 font-medium">Diaconia</span>
          )}
          {item.ministries?.map((m) => (
            <span key={m.ministryId} className="px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-800 font-medium">
              {m.ministry.name}
            </span>
          ))}
          {item.bibleSchoolClass && (
            <span className="px-2 py-0.5 rounded-full text-xs bg-amber-100 text-amber-800 font-medium">EBD</span>
          )}
          {!item.societies?.length && !item.council && !item.diaconate && !item.ministries?.length && !item.bibleSchoolClass && (
